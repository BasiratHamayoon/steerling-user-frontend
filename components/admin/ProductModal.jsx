...
export default function ProductModal({ isOpen, onClose, product, onSave }) {
  ...
  const resetForm = () => {
    ...
    setExistingImages([]); setNewImages([]); setImagePreviews([]);
  };

  const handleImageUpload = (e) => {
    const files = Array.from(e.target.files);
    if (newImages.length + files.length > 10) return alert('Maximum 10 images allowed');
  ...
  const handleAddSpec = () => {
    if (specInput.key.trim() && specInput.value.trim()) {
      setFormData({ ...formData, specifications: { ...formData.specifications, [specInput.key.trim()]: specInput.value.trim() } });
      setSpecInput({ key: '', value: '' });
    }
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Removed price check from validation
    if (!formData.title || !formData.category) return alert('Please fill required fields (Title, Category)');
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
          ...
          <form id="productForm" onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                />
                <div className="group">
                   <label className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1">Category <span className="text-red-500">*</span></label>
                   <select
              ...
              <div className="group">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1 block">Description</label>
                  <textarea
                    rows="4"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <InputGroup label="Material" value={formData.material} onChange={e => setFormData({...formData, material: e.target.value})} />
                 <InputGroup label="Diameter" value={formData.diameter} onChange={e => setFormData({...formData, diameter: e.target.value})} />
                 <InputGroup label="Compatibility" value={formData.compatibility} onChange={e => setFormData({...formData, compatibility: e.target.value})} />
              </div>

              {/* Dynamic Specs */}
              <div className="bg-gray-800/30 p-4 rounded-xl border border-gray-800">
                <div className="flex gap-2 mb-4">
                  <input placeholder="Spec Name (e.g. Stitch Color)" value={specInput.key} onChange={e => setSpecInput({...specInput, key: e.target.value})} className="flex-1 bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 text-sm text-white" />
                  <input placeholder="Value (e.g. Red)" value={specInput.value} onChange={e => setSpecInput({...specInput, value: e.target.value})} className="flex-1 bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 text-sm text-white" />
                  <button type="button" onClick={handleAddSpec} className="px-4 bg-[#0295E6] hover:bg-[#0284c6] text-white rounded-lg text-sm font-bold">Add</button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(formData.specifications).map(([k, v]) => (
                    <span key={k} className="inline-flex items-center gap-2 px-3 py-1 bg-gray-900 border border-gray-700 rounded-full text-xs text-gray-300">
                      ...
                      <button type="button" onClick={() => handleRemoveSpec(k)} className="hover:text-red-400"><FaTimes /></button>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-1">Supports JPG, PNG, WEBP (Max 10)</p>
              </div>
                  ...
                  {imagePreviews.map((src, idx) => (
                    <div key={idx} className="relative aspect-square group rounded-xl overflow-hidden border border-gray-700">
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button type="button" onClick={() => handleRemoveImage(idx)} className="p-2 bg-red-600 rounded-full text-white hover:bg-red-700"><FaTrash size={12} /></button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="px-8 py-5 bg-gray-900/95 backdrop-blur border-t border-gray-800 flex justify-end gap-4 z-10">
          <button onClick={onClose} className="px-6 py-2.5 rounded-xl border border-gray-700 text-gray-300 hover:bg-gray-800 transition-colors">Cancel</button>
          <button form="productForm" type="submit" className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-[#0295E6] to-[#0077b6] text-white font-bold shadow-lg hover:shadow-blue-500/20 hover:scale-105 transition-all">
